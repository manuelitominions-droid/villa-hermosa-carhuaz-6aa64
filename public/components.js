...
function createRegistrosTable(registros) {
    ...
    registros.forEach(r => {
        // CORREGIDO: Usar numeric_id si existe, sino usar un número basado en el índice
        const displayId = r.numeric_id || registros.indexOf(r) + 1;
        ...
        // CORREGIDO: Calcular estado de cuotas
        const estadoHTML = renderEstadoHTML(r);
        
        // CORREGIDO: Usar el ID de Firebase para las funciones, pero mostrar numeric_id
        const opciones = r.forma_pago === 'cuotas' 
            ? `<button onclick="showCuotasModal('${r.id}')" class="text-blue-600 hover:text-blue-800 mr-2" title="Ver cuotas">Cuotas</button>`
            : `<button onclick="showCuotasModal('${r.id}')" class="text-blue-600 hover:text-blue-800 mr-2" title="Ver detalles">Contado</button>`;
        
        const _auth = getAuth();
        const editBtn = (_auth && _auth.isAdmin && _auth.isAdmin()) 
            ? `<button onclick="showEditCuotasModal('${r.id}')" class="mr-2 text-orange-500 hover:text-orange-600" title="Editar cuotas" style="line-height:0">
...
// NUEVA FUNCIÓN: Render HTML para la columna Estado
async function renderEstadoHTML(registro) {
    ...
    try {
        ...
        // Para cuotas, calcular pendientes
        const saldo = db.getSaldoPendienteByRegistro(registro.id);
        
        if (saldo.pendingCount === 0) {
            return '<span class="text-green-600 font-semibold">Al día</span>';
        } else {
            return `<span class="text-red-600 font-semibold">Debe ${saldo.pendingCount} cuota${saldo.pendingCount > 1 ? 's' : ''}</span>`;
        }
    } catch (e) {
...
async function updateRegistroRow(registroId) {
    try {
        ...
        if (estadoCell) {
            estadoCell.innerHTML = await renderEstadoHTML(registro);
        }
        ...
        if (opcionesCell) {
            ...
            const opcionesHTML = registro.forma_pago === 'cuotas'
                ? `<button onclick="showCuotasModal('${registro.id}')" class="text-blue-600 hover:text-blue-800 mr-2" title="Ver cuotas">Cuotas</button>`
                : `<button onclick="showCuotasModal('${registro.id}')" class="text-blue-600 hover:text-blue-800 mr-2" title="Ver detalles">Contado</button>`;
            const editBtn = (_auth && _auth.isAdmin && _auth.isAdmin())
                ? `<button onclick="showEditCuotasModal('${registro.id}')" class="text-orange-600 hover:text-orange-800 mr-2" title="Editar cuotas">✏️</button>` : '';
            const deleteBtn = (_auth && _auth.isAdmin && _auth.isAdmin())
...
async function showEditCuotasModal(registroId) {
    const _auth = getAuth();
    if (!(_auth && _auth.isAdmin && _auth.isAdmin())) {
...
async function aplicarMontoATodas(registroId) {
    ...
    cuotasNormales.forEach((c, index) => {
        const input = document.getElementById(`cuota_${c.numero}`);
        if (input) {
            if (index === cuotasNormales.length - 1) {
                // Última cuota: calcular el saldo restante
                const montoAcumulado = monto * (cuotasNormales.length - 1);
...
async function ajustarCuotasAutomaticamente(registroId) {
    const db = await waitForDatabase();
    const registro = db.getRegistroById(registroId);
    const cuotas = await db.getCuotasByRegistroId(registroId);
    const cuotasNormales = cuotas.filter(c => c.numero > 0);
...
// Modal para ver cuotas - ACTUALIZADO con funciones corregidas
async function showCuotasModal(registroId) {
    const db = await waitForDatabase();
    const registro = db.getRegistroById(registroId);
    const cuotas = await db.getCuotasByRegistroId(registroId);
...
async function handleEditCuotas(event, registroId) {
    ...
    for (let i = 0; i < cuotasNormales.length; i++) {
        const c = cuotasNormales[i];
        if (montosPersonalizados[i] !== undefined && fechasPersonalizadas[i] !== undefined) {
            await db.updateCuota(c.id, { 
                monto: montosPersonalizados[i],
                fecha_vencimiento: fechasPersonalizadas[i]
            });
        }
    }
...
if (typeof renderEstadoHTML === 'function') window.renderEstadoHTML = renderEstadoHTML;
...
console.log('✅ Components.js cargado con funciones corregidas');
